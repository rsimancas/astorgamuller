function loadMovs() {
    ...
    var source =
    {
        ...
        datafields: [
            ...
            { name:'MovComentarioInterno', type:'string'},
    ...
    $("#jqxgridMovs").jqxGrid(
    {
        ...
        updatedelay: 1,
        pagermode: 'simple',
        pagesize: (urlparams && urlparams.rows) ? urlparams.rows : 24,
        virtualmode: true,
        autorowheight: false,
        //enabletooltips: true,
        showtoolbar: true,
        ...
        columns: [
            ...
            { text: 'Viaje', cellsrenderer: cellsrenderer, datafield: 'MovViaje', align: 'center', width:'5%'},
            ...
            { text: 'Ent.', datafield: 'MovFechaEntregado', align: 'center', width: '3.8%',
                cellsrenderer: function (index, datafield, value, defaultHtml, column, rowdata) {
            ...
            { text: 'Cliente', datafield: 'x_Cliente', cellsalign: 'center', align: 'center', width:'24%', cellsrenderer: cellsrenderercliente},
            { text: 'E. Info', datafield: 'x_EstatusInfo', cellsalign: 'center', align: 'center', width: '9%',
                cellsrenderer: function(index, datafield, value, defaultHtml, column, rowdata) {
                    var strEstatusInfo = (!String.isNullOrEmpty(rowdata.x_EstatusInfo)) ?  rowdata.x_EstatusInfo : null,
                        ...
                        strComentarioInterno = (!String.isNullOrEmpty(rowdata.MovComentarioInterno)) ? rowdata.MovComentarioInterno : null;
                        strUbicacionTransito = (!String.isNullOrEmpty(rowdata.MovUbicacionTransito)) ? rowdata.MovUbicacionTransito : null;

                    strEstatusInfo = (!String.isNullOrEmpty(rowdata.x_EstatusInfo) && !String.isNullOrEmpty(rowdata.MovComentario)) ? strEstatusInfo + ' | ' + strComentario : strEstatusInfo;
                    strEstatusInfo = (String.isNullOrEmpty(rowdata.x_EstatusInfo) && !String.isNullOrEmpty(rowdata.MovComentario)) ? strComentario : strEstatusInfo;
                    strEstatusInfo = (!String.isNullOrEmpty(rowdata.x_EstatusInfo) && String.isNullOrEmpty(rowdata.MovComentario)) ? rowdata.x_EstatusInfo : strEstatusInfo;
                    strEstatusInfo = (String.isNullOrEmpty(rowdata.x_EstatusInfo) && String.isNullOrEmpty(rowdata.MovComentario)) ? null : strEstatusInfo;

                    strEstatusInfo = (!String.isNullOrEmpty(strEstatusInfo) && !String.isNullOrEmpty(strComentarioInterno)) ? strEstatusInfo + ' | ' + strComentarioInterno : strEstatusInfo;
                    strEstatusInfo = (String.isNullOrEmpty(strEstatusInfo) && !String.isNullOrEmpty(strComentarioInterno)) ? strComentarioInterno : strEstatusInfo;

                    strEstatusInfo = (!String.isNullOrEmpty(strEstatusInfo) && !String.isNullOrEmpty(strUbicacionTransito)) ? strEstatusInfo + ' | ' + strUbicacionTransito : strEstatusInfo;
                    strEstatusInfo = (String.isNullOrEmpty(strEstatusInfo) && !String.isNullOrEmpty(strUbicacionTransito)) ? strUbicacionTransito : strEstatusInfo;
            ...
            { text:'Salida', datafield:'x_HoraSalida',  align: 'center', width: '5.5%',
                cellsrenderer: function (index, datafield, value, defaultHtml, column, rowdata) {

                    var returnVal = (String.isNullOrEmpty(value)) ? "-" : value;

                    returnVal = (String.isNullOrEmpty(value)) ? returnVal : rowdata.x_FechaSalida + " " + value;
            ...
            { text: 'Sta', datafield: 'x_Estatus', cellsalign: 'center', align: 'center', width: '2.5%',
                cellsrenderer: function(index, datafield, value, defaultHtml, column, rowdata) {
                    ...
                    iconCls = (value === 'AUDITORIA')  ? 'fa fa-pencil' : iconCls;
            ...
            { text:'Img', datafield:'x_Attachments', cellsalign:'center', align:'center', width: '2.5%',
                cellsrenderer: function (index, datafield, value, defaultHtml, column, rowdata) {
                    //if(rowdata.MovFechaCompletado && rowdata.x_DL) {
                    var strViaje = "'" + rowdata.MovViaje + "'",
                        returnVal = "";

                    if(value > 0) 
                        returnVal = '<div style="vertical-align:middle; padding-left:5px; padding-top: 5px; "><p align="center"><a class="fa fa-camera" href="#" onclick="_CreateWindow('+rowdata.MovId+', '+strViaje+');"></a></p></div>';
                    //} else {
                    //    returnVal = '';
                    //}
                    var element = $(defaultHtml);
    ...
    $("#jqxgridMovs").on("bindingcomplete", function (event) {
        ...
        var refreshData = function() {
            //var paginginformation = $('#jqxgridMovs').jqxGrid('getpaginginformation');
            //console.log(paginginformation);
...
function loadEquipos() {
    ...
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        ...
        color = (value === "TRANSITO") ? '#DADA00' : color;
        color = (value === "CLIENTE") ? '#03c3fa' : color;
        color = (value === "MANTENIMIENTO") ? '#E44444' : color; //#804515
        color = (value === "AUDITORIA") ? '#3C4B9C' : color;
        ...
        if(value === "REPROGRAMADO") element.addClass("fa fa-calendar-check-o");
        ...
        if(value === "AUDITORIA") element.addClass("fa fa-pencil");
...
function loadResumenSemanaMeses() {
    ...
    var dataAdapter = new $.jqx.dataAdapter(source, {
        beforeLoadComplete: function (records) {
            ...
            var addRecords = [
                {Descripcion: "Turno 1", Cantidad: 0, Promedio: 0, Cavas: 0}
            ];

            return addRecords.concat(records);
    ...
    $("#jqxgridResumenSemanaMeses").jqxGrid(
    {
        ...
        rendergridrows: function()
        {
            ...
            //return dataAdapter.records;     
            return records.concat(dataAdapter.records);
        },
        columns: [
...
$(document).ready(function () {
    ...
    htmlLeyenda = '<p>' +
        ...
        '<span class="fa fa-calendar-check-o" aria-hidden="false"  style="color:#8A458A;">=REPROGRAMADO</span><br>' +
        '</p>';
