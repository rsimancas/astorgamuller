Ext.define('Muller.view.Distribucion', {
    ...
    initComponent: function () {
        ...
        storeClientesReparto = Ext.create('Muller.store.Clientes').load({ params: { page: 0, start: 0, limit: 0 } });
        storeClientes = Ext.create('Muller.store.Clientes').load({ params: { page: 0, start: 0, limit: 0 } });
        storeCiudadesReparto = Ext.create('Muller.store.Ciudades').load({ params: { iata: 1, page: 0, start: 0, limit: 0 } });
        storeCiudades = Ext.create('Muller.store.Ciudades').load({ params: { iata: 1, page: 0, start: 0, limit: 0 } });
        storeEstatus = Ext.create('Muller.store.Estatus').load({ params: { tipo: 'D', page: 0, start: 0, limit: 0 } });
        storeChoferes = Ext.create('Muller.store.Choferes').load({ params: { page: 0, start: 0, limit: 0 } });
        storeEquipos = null;
        storeTabulador = Ext.create('Muller.store.Tabulador').load({ params: { page: 0, start: 0, limit: 0 } });
        storeRepartos = null;

        var storeUbicaciones = Ext.create('Muller.store.Ubicaciones').load();
                                    ...
                                    tbar: [
                                        {
                                            ...
                                            handler: function () {
                                                ...
                                                var r = Ext.create('Muller.model.Repartos', {
                                                    RepartoCantidad: 0,
    ...
    onAfterLoadRecord: function (tool, record) {
        ...
        storeEquipos = Ext.create('Muller.store.Equipos', { remoteFilter: false }).load({
            ...
            callback: function () {
                ...
                //storeEstatus = Ext.create('Muller.store.Estatus').load({params:{tipo:'D', orden: curorden, fallido: esFallido, page:0, start: 0, limit: 0}, 
                storeEstatus = Ext.create('Muller.store.Estatus', { remoteSort: false }).load({
                    params: { tipo: 'D', page: 0, start: 0, limit: 0 },
                    callback: function () {
    ...
    onAddClick: function (tool, record) {
        ...
        //toolbar.bindStore(Ext.create('Muller.store.Movimientos'));
        //toolbar.store.add(r);
    ...
    onCheckRepartos: function () {
        ...
        var form = Ext.create('Muller.view.PromptRepartos', {
            height: 185,
            modal: true,
            width: 334,
            frameHeader: true,
    ...
    validRepartos: function () {
        ...
        if (count < totalRepartos) {
            Ext.Msg.alert('Alerta', 'Debe completar la cantidad de repartos');
            return false;
        }
