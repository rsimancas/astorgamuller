Ext.define('Ext.ux.field.AutocompleteField', {
	...
	initialize: function() {
		...
		var doSearchWithTimeout = function() {
			...
			searchTimeout = setTimeout(function() {
				...
				that.resultsStore.getProxy().setUrl(that.config.config.proxy.url);
				that.resultsStore.getProxy().setExtraParam('query', that.getValue(true));
		...
		this.resultsList.on('itemtap', function(self, index, target, record) {
			...
			//console.log(that, record.get('name'));
			that.setValue(record.get('name'), record.get('name'));
