...
{
    public class GastosGruposItemsRepository : IGastosGruposItemsRepository
    {
        public GastosGruposItemsRepository()
        {
            
        }

        #region "GastosGrupoItem"

        public IList<GastosGrupoItem> GetList(int page, int start, int limit, ref int totalRecords)
        {
            limit = limit + start;
            ...
            string sql = "SELECT * FROM ( " +
                         "SELECT *, " +
                         "  ROW_NUMBER() OVER (ORDER BY GGrupoItemNombre) as row,  " +
                         "  IsNull((select count(*) from GastosGruposItems),0)  as TotalRecords   " +
                         " FROM GastosGruposItems) a  " +
                         where +
                         " ORDER BY row";
        ...
        public GastosGrupoItem Get(int id)
        {
            SqlConnection oConn = null;
            ...
            GastosGrupoItem data = Get(id, oConn);
        ...
        public GastosGrupoItem Add(GastosGrupoItem cliente, ref string msgError)
        {
            SqlConnection oConn = null;
            ...
            string sql = "INSERT INTO GastosGruposItems ({0}) VALUES ({1}) " +
                "SELECT SCOPE_IDENTITY()";

            cliente.GGrupoItemCreado = DateTime.Now;
            EnumExtension.setListValues(cliente, "GGrupoItemId", ref sql);
            ...
            GastosGrupoItem data = Get(keyGenerated, oConn);
        ...
        private GastosGrupoItem Get(int id, SqlConnection oConn)
        {
            string sql = "SELECT * FROM GastosGruposItems " +
                         " WHERE (GGrupoItemId = @id)";
            ...
            return data.FirstOrDefault<GastosGrupoItem>();
        }

        public bool Remove(GastosGrupoItem cliente, ref string msgError)
        {
            SqlConnection oConn = null;
        ...
        private bool Remove(GastosGrupoItem cliente, SqlConnection oConn)
        {
            string sql = "DELETE FROM GastosGruposItems " +
                         " WHERE (GGrupoItemId = {0})";

            sql = String.Format(sql, cliente.GGrupoItemId);
